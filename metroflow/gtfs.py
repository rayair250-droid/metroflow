...
def export_scenario(
    ...
    """Render a self-contained scenario YAML for one GTFS route/direction.

    The line (stations, names, per-segment run-times) is REAL, derived from the
    feed's timetable. Everything else -- demand profile, incidents, fleet -- is
    inherited from ``base_scenario`` (or the defaults) and stays SYNTHETIC:
    GTFS describes service, not passenger counts. The generated header says so,
    so a scenario can never silently pass timetable data off as demand data.
    """
    import yaml

    from metroflow.config import config_to_dict, load_config, validate_config

    cfg = load_config(base_scenario)
    built = apply_gtfs(cfg, directory, route_id, direction_id)
    cfg.name = name or f"gtfs_{route_id}_dir{direction_id}"
    if demand_profile is not None:
        cfg.demand.profile = demand_profile
        validate_config(cfg)  # reject unknown profiles at export time, not run time

    header_lines = [
        f"MetroFlow scenario generated by `metroflow gtfs-export` -- route {built.route_id}",
        f"({built.route_label}), direction {built.direction_id}: "
        f"{built.station_names[0]} -> {built.station_names[-1]}, "
        f"{built.line.n_stations} stations.",
        "",
        "REAL (timetable-derived): station list/order and per-segment run-times,",
        "  averaged over every trip of this route/direction in the feed.",
        "SYNTHETIC (inherited from the base scenario, NOT in GTFS): passenger",
        "  demand profile, incident rates, fleet size, controller settings.",
    ]
    if demand_profile is not None:
        header_lines += [
            "",
            f"Demand uses the named '{demand_profile}' spatial profile — a plausible",
            "  shape for this line type, still synthetic, not measured ridership.",
        ]
    if source_note:
        header_lines += ["", f"Source: {source_note}"]
    header = "".join(f"# {line}".rstrip() + "\n" for line in header_lines)

    body = yaml.safe_dump(config_to_dict(cfg), sort_keys=False, allow_unicode=True)
    return header + body
